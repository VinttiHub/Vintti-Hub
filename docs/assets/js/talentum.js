...
async function selectOpportunity(opp) {
  ...
  highlightOpportunity(opp.opportunity_id);
  setChatStatus("Loading");
  setFiltersStatus("Extracting");
  setCandidateSubtitle("Syncing applicants and filters…");
  els.candidatesGrid.innerHTML = "";
  els.candidatesEmpty.textContent = "Loading applicants…";
  els.candidatesEmpty.style.display = "block";

  const oppName = opp.opp_position_name || "Opportunity";
  if (els.chatSubtitle) {
    els.chatSubtitle.textContent = `Filters for ${oppName}`;
  }

  try {
    await extractFiltersFromOpportunity(opp.opportunity_id);
  ...
  try {
    await loadApplicants(opp.opportunity_id);
  } catch (err) {
...
async function extractFiltersFromOpportunity(opportunityId) {
...
async function loadOpportunities() {
  ...
  if (visible.length) {
    await selectOpportunity(visible[0]);
  }
}

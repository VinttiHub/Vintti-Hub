...
def query(filters: dict, *_args, **_kwargs) -> tuple[str, dict]:
    mes = (
        _parse_date(filters.get("fecha_placement_time_repl"))
    ...
    sql = """
        ...
        WHERE 1=1
          AND DATE_TRUNC('month', b.close_d)::date = mo.mes_pick
          AND (%(desde)s::date IS NULL OR b.close_d >= %(desde)s::date)
          AND (%(hasta)s::date IS NULL OR b.close_d <  (%(hasta)s::date + INTERVAL '1 day'))
...
DATASET = {
    "key": "placement_time_repl_month_detail",
    "label": "Tiempo de colocación (Replacement) — Detalle del mes",
    "dimensions": [

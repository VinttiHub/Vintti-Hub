...
def query(filters: dict, *_args, **_kwargs) -> tuple[str, dict]:
    ...
    sql = """
        ...
        SELECT
          TO_CHAR(p.corte_d, 'YYYY-MM-DD')              AS fecha_corte,
          COUNT(*)::int                                  AS total_batches_30d,
          COUNT(DISTINCT opportunity_id)::int            AS opps_con_batches_30d,
          COALESCE(ROUND(AVG(dias_entrega))::int, 0)     AS avg_dias_entrega_30d
        FROM detallado
        CROSS JOIN params_final p
        GROUP BY p.corte_d;
    """
...
DATASET = {
    "key": "batch_delivery_time_30d_summary",
    "label": "Tiempo promedio en entregar un batch — Ventana 30 días",
    "dimensions": [
        {"key": "fecha_corte", "label": "Fecha corte", "type": "date"},
    ],
    "measures": [
        {"key": "total_batches_30d", "label": "Total batches", "type": "number"},
        {"key": "opps_con_batches_30d", "label": "Opps con batches", "type": "number"},
        {"key": "avg_dias_entrega_30d", "label": "Días promedio entrega", "type": "number"},
    ],
    "default_filters": {},
    "query": query,
}

...
def query(filters: dict, *_args, **_kwargs) -> tuple[str, dict]:
    ...
    mes = (
        _parse_date(filters.get("mes_batch_delivery"))
        or _parse_date(filters.get("Batch_M"))
    ...
    sql = """
        ...
        detalle AS (
          SELECT
            bp.batch_number,
            bp.batch_d,
            DATE_TRUNC('month', bp.batch_d)::date AS mes_batch,
            bp.opportunity_id,
            o.client_name,
            o.opp_position_name,
            o.opp_model,
        ...
        SELECT
          d.batch_number,
          TO_CHAR(d.batch_d,      'YYYY-MM-DD') AS batch_d,
          TO_CHAR(d.mes_batch,    'YYYY-MM-DD') AS mes_batch,
          d.opportunity_id::text                AS opportunity_id,
          d.client_name,
          d.opp_position_name,
          d.opp_model,
          d.opp_stage,
          TO_CHAR(d.fecha_pedido, 'YYYY-MM-DD') AS fecha_pedido,
          d.dias_entrega
        FROM detalle d
        CROSS JOIN mes_objetivo mo
        WHERE d.mes_batch = mo.mes_pick
        ORDER BY d.batch_d, d.opportunity_id, d.batch_number;
    """

    return sql, {"modelo": modelo, "cliente": cliente, "mes": mes}


DATASET = {
    "key": "batch_delivery_time_month_detail",
    "label": "Tiempo promedio en entregar un batch — detalle por mes",
    "dimensions": [

...
def handle_candidate_hire_data(candidate_id):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if not row:
                ...
                return jsonify({
                    ...
                    'reference_1_company': candidate_reference_row.get('reference_1_company'),
                    ...
                    'reference_2_company': candidate_reference_row.get('reference_2_company'),
            ...
            return jsonify({
                ...
                'reference_1_company': row['reference_1_company'] or candidate_reference_row.get('reference_1_company'),
                ...
                'reference_2_company': row['reference_2_company'] or candidate_reference_row.get('reference_2_company'),
        ...
        mapping = {
            ...
            'reference_1_position': 'reference_1_position',
            ...
            'reference_2_position': 'reference_2_position',
...
# --- LinkedIn pegado a mano ---------------------------------------------------------
# El LinkedIn que tiene Coresignal es una COPIA que ellos refrescan cuando quieren: la de
# la review 196 era de oct-2025 y tenía un trabajo que el candidato ya había borrado. El
# CV Review lo compara contra el CV, así que una copia vieja acusa omisiones que no son.
# La recruiter pega la sección Experiencia del LinkedIn de hoy y queda fechada: el CV
# Review usa la más nueva de las dos (cv_review_ai.linkedin_input). También la usa el
# generador de CV, que ya prefería linkedin_scrapper sobre coresignal_scrapper.
#
# La fecha va en una columna aparte y NO en el PATCH genérico: el modal de React también
# escribe linkedin_scrapper y ése no es necesariamente el LinkedIn de hoy.
_LINKEDIN_PASTE_COLUMN_READY = False


def ensure_linkedin_paste_column(cursor):
    """Crea candidates.linkedin_scrapper_at si falta. Mira el catálogo primero para no
    pedir un lock sobre candidates en cada request."""
    global _LINKEDIN_PASTE_COLUMN_READY
    if _LINKEDIN_PASTE_COLUMN_READY:
        return
    cursor.execute("""
        SELECT 1 FROM information_schema.columns
         WHERE table_name = 'candidates' AND column_name = 'linkedin_scrapper_at'
    """)
    if not cursor.fetchone():
        # Columna nullable sin default: es sólo metadata, pero el ALTER pide un lock
        # exclusivo. Con lock_timeout no se queda en la cola bloqueando a todos si hay una
        # query larga sobre candidates; falla, y el próximo request reintenta.
        cursor.execute("SET LOCAL lock_timeout = '3s'")
        cursor.execute("ALTER TABLE candidates ADD COLUMN IF NOT EXISTS linkedin_scrapper_at TIMESTAMPTZ")
    _LINKEDIN_PASTE_COLUMN_READY = True


def _linkedin_source_state(cursor, candidate_id):
    from utils.coresignal_profile import load_profile, profile_as_of
    from utils.cv_review_ai import LINKEDIN_STALE_DAYS
    cursor.execute("""
        SELECT COALESCE(linkedin_scrapper, '') AS pasted,
               linkedin_scrapper_at, COALESCE(coresignal_scrapper, '') AS cs
          FROM candidates WHERE candidate_id = %s
    """, (candidate_id,))
    row = cursor.fetchone()
    if not row:
        return None
    profile = load_profile(row["cs"])
    cs_as_of = profile_as_of(profile) if profile else None
    pasted_at = row["linkedin_scrapper_at"]
    return {
        "pasted_text": row["pasted"] if pasted_at else "",
        "pasted_at": pasted_at.isoformat() if pasted_at else None,
        "coresignal_as_of": cs_as_of.isoformat() if cs_as_of else None,
        "stale_days": LINKEDIN_STALE_DAYS,
    }


@bp.route('/candidates/linkedin_status', methods=['GET'])
def candidates_linkedin_status():
    """El mismo estado que GET .../linkedin_paste, para N candidatos en un request. Lo usa
    el envío por batch a Sales Review para pedir el LinkedIn de los que lo tienen viejo.

    ?ids=1,2,3 → {"stale_days": 90, "candidates": {"1": {...}, ...}}. No devuelve el texto
    pegado (sólo si hay y su fecha): con 10 candidatos serían cientos de KB para nada.
    """
    from utils.coresignal_profile import load_profile, profile_as_of
    from utils.cv_review_ai import LINKEDIN_STALE_DAYS
    ids = []
    ...
    ids = ids[:100]
    if not ids:
        return jsonify({'stale_days': LINKEDIN_STALE_DAYS, 'candidates': {}})
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("""
        ...
        return jsonify({'stale_days': LINKEDIN_STALE_DAYS, 'candidates': out})
    except Exception as exc:
        logging.exception("linkedin_status failed")
        return jsonify({'error': str(exc)}), 500
    finally:
        cursor.close()
        conn.close()
...
@bp.route('/candidates/<int:candidate_id>/linkedin_paste', methods=['GET', 'PUT', 'OPTIONS'])
def candidate_linkedin_paste(candidate_id):
    if request.method == 'OPTIONS':
        return ('', 204)
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        ensure_linkedin_paste_column(cursor)
        if request.method == 'PUT':
            text = str((request.get_json(silent=True) or {}).get('text') or '').strip()
            # Lo primero que hace todo el mundo es pegar el link (pasó el mismo día que se
            # lanzó). Un link no sirve: no podemos leer LinkedIn, por eso existe este campo.
            if re.fullmatch(r'(https?://)?(www\.)?[\w.-]*linkedin\.com/\S*', text, re.I):
                return jsonify({'error': "That's the link — we need the text. Open the profile, "
                                         "select the whole Experience section with the mouse, "
                                         "copy it and paste it here (LinkedIn doesn't let us "
                                         "read the profile from the link)."}), 400
            if text and len(text) < 200:
                return jsonify({'error': 'Paste the whole Experience section of the profile '
                                         '(it looks too short to be one).'}), 400
            cursor.execute("""
                UPDATE candidates
                   SET linkedin_scrapper = %s,
                       linkedin_scrapper_at = CASE WHEN %s = '' THEN NULL ELSE NOW() END
                 WHERE candidate_id = %s
            """, (text or None, text, candidate_id))
            if cursor.rowcount == 0:
                conn.rollback()
                return jsonify({'error': 'Candidate not found'}), 404
        state = _linkedin_source_state(cursor, candidate_id)
        conn.commit()
        if state is None:
            return jsonify({'error': 'Candidate not found'}), 404
        ...
        conn.rollback()
        logging.exception("linkedin_paste failed for candidate %s", candidate_id)
        return jsonify({'error': str(exc)}), 500
    finally:
        cursor.close()
        conn.close()
